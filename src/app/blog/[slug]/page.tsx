  ...
  return (
    <div className="min-h-screen bg-bg-base">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-3">
        <BackButton />
        <nav className="flex items-center gap-2 text-sm text-text-muted" aria-label="Хлебные крошки">
          <Link href="/" className="hover:text-rose-600 dark:hover:text-rose-400 transition-colors">Главная</Link>
          <span>/</span>
          <Link href="/blog" className="hover:text-rose-600 dark:hover:text-rose-400 transition-colors">Блог</Link>
          <span>/</span>
          <span className="text-text-body line-clamp-1">{post.title}</span>
        </nav>
      </div>
      ...
      <header className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-6">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <span className="inline-block text-sm font-medium bg-rose-600/10 border border-rose-500/20 text-rose-700 dark:text-rose-400 px-3 py-1 rounded-full">
            {post.category}
          </span>
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-text-head leading-tight mb-5">
          {post.title}
        </h1>
        <p className="text-lg text-text-body leading-relaxed border-l-4 border-rose-700/50 pl-4">
          {post.description}
        </p>
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-5">
            {post.tags.map((tag) => (
              <span key={tag} className="text-xs text-rose-700 dark:text-rose-400 bg-rose-600/10 border border-rose-500/20 rounded-full px-2.5 py-1">
                #{tag}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div className="bg-bg-raised border border-border-md rounded-2xl p-6 sm:p-8 text-center">
          <h3 className="text-xl font-bold text-text-soft mb-2">
            Хотите персональный совет?
          </h3>
          <p className="text-text-body mb-5 text-sm">
      ...
      {related.length > 0 && (
        <section className="bg-bg-deep border-t border-border py-14">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-text-head mb-8">Читайте также</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {related.map((p) => (
                <Link
                  key={p.slug}
                  href={`/blog/${p.slug}`}
                  className="group bg-bg-raised border border-border hover:border-border-md rounded-2xl p-5 transition-all duration-200 hover:bg-bg-card"
                >
                  ...
                  <h3 className="font-bold text-text-soft text-sm leading-snug group-hover:text-rose-700 dark:group-hover:text-rose-300 transition-colors line-clamp-2 mb-2">
                    {p.title}
                  </h3>
